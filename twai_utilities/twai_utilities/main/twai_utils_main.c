...
#if CONFIG_ESP_CONSOLE_UART
    /* Primary console/REPL is on UART; also accept commands over USB
     * Serial/JTAG so both interfaces are interactive at the same time.
     * (esp_console only supports one esp_console_new_repl_* instance, so
     * the second interface is driven manually via esp_console_run().) */
    console_usb_serial_jtag_start();
#endif
}
